...
static NSString *const badgeInitClr = @"#666666";
static NSString *const badgeNewClr = @"#fd373b";
...
- (void)pp_configBadgeWithNumber:(NSNumber *)number andFrame:(CGRect)badgeFrame andFont:(UIFont *)font {
    if (!self.badgeLabel) {
//        self.badgeLabel = [PPBadgeLabel defaultBadgeLabelWithFrame:badgeFrame];
        self.badgeLabel = [PPBadgeLabel defaultBadgeLabelWithFrame:badgeFrame andFont:font];
        [self addSubview:self.badgeLabel];
        [self bringSubviewToFront:self.badgeLabel];
    } else {
        [self.badgeLabel setFrame:badgeFrame];
    }
    
    [self pp_showBadge];
    self.badgeLabel.layer.backgroundColor = [UIColor colorWithHexString:(number.intValue == 0 ? badgeInitClr: badgeNewClr)].CGColor;
    self.badgeLabel.text = [NSString stringWithFormat:@"%@",(number.intValue >= 100 ? @"99+": number)];
}
